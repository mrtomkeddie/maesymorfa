...
function startGame() {
  // Reset loop to prevent double-speed or stopped-loop issues
  if (gameLoopId) {
...
function loop() {
  if (!gameRunning && !showingHighScores && !enteringName && !showingWelcome) {
    ...
    ctx.fillText("[ TAP / SPACE to Jump ]", canvasWidth / 2, canvasHeight / 2 + 20);
    ctx.restore();
...
fullscreenButton.addEventListener("click", () => {
  if (!document.fullscreenElement && !document.webkitFullscreenElement) {
    ...
    if (gameContainer.requestFullscreen) {
      ...
      // iOS often only supports fullscreen on video or sometimes canvas
